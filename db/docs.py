...
class DocumentManager:
    ...
    def search_docs(self, query):
        ...
        result = (
            self.docs.query.hybrid(
                query=query,
                query_properties=["author",  "responder_category", "support", "motivation", "regulation",  "perceived_impact", "regulator_trust", "file_name"],
                vector=query_vector,
                alpha=0.75,
                limit=100
            )            
    ...
    def doc_getter(self, id):
        self.init_client()        
        doc = self.client.data_object.get_by_id(id, class_name="Submission")
        return doc
    ...
    def update_doc(self, id, data_obj: Document):
        self.init_client()
        doc = self.client.data_object.get_by_id(id, class_name="Submission")
        doc.properties = data_obj
        self.docs.data.update(doc)
        return doc
